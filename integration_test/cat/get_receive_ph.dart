// ignore_for_file: unused_local_variable

import 'package:chia_crypto_utils/chia_crypto_utils.dart';
import 'package:test/test.dart';

const puzzleRevealHEx =
    '';
const puzzleRevealHEx2 =
    '';
const puzzleRevealHEx3 =
    '';

Future<void> main() async {
  test('NFT uncurried test', () async {
    final toFind =
        const Address('xch1zhnyj5fnf0dpspr0gtuxrkhq52y0v4nznlrr987tyn4dc205eyfstsqtrj').toPuzzlehash();
    final toFind2 =
        const Address('xch1gfhwjzwkz3zxlg4k577fh57c5kz7q0csl7tht3dzte9ag779thxs2cwpgr').toPuzzlehash();
    final puzzleReveal = Program.deserializeHex(puzzleRevealHEx);
    final uncurred = puzzleReveal.toList().first.rest().first().toList()[2].rest().first().atom;
    print(uncurred);

    try {
      final toFind =
          const Address('txch1ht3yzch0h4tglzdu0g6q0x9xzxxlqxy7h83ls6tme63847vl3aussy886w').toPuzzlehash();
      final toFind2 =
          const Address('txch19ytuhga7mn0vk6qdljt7pxzsy463y8rafz32kh2apyf74ylfje5sdd5csy').toPuzzlehash();
      final puzzleReveal = Program.deserializeHex(puzzleRevealHEx2);
      final first = puzzleReveal.toList().first.first().rest().first();
      final temp = puzzleReveal.toList().first.toList().first.toList()[1].toList();
      final uncurred = puzzleReveal.toList().first.rest().first().toList()[2].rest().first().atom;
      print(uncurred);
    } catch (e) {}
    try {
      final toFind =
          const Address('txch1n4ezjxefa3qv5zs0p55y8hy2788lzv4dr5xnecr6gunugphelagqdjwewp').toPuzzlehash();
      final toFind2 =
          const Address('txch1heptx67wm9r835ugdd07qk7frecxahzfrrrfrd0z7kznz78q2nws7ukzlr').toPuzzlehash();
      final puzzleReveal = Program.deserializeHex(puzzleRevealHEx3);
      final index = puzzleRevealHEx3.indexOf(toFind2.toHex());
      final temp = puzzleReveal.toList();
      final uncurred = puzzleReveal.toList().first.rest().first().toList()[2].rest().first().atom;
      print(uncurred);
    } catch (e) {}
  });
}
