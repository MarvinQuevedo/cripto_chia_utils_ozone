...
Puzzlehash strToTailHash(String tailStr) {
  return Program.list([
    Program.fromInt(3),
    Program.list([]),
    Program.list([Program.fromInt(1), Program.fromString(tailStr)]),
    Program.list([])
  ]).hash();
}
...
Future<void> main() async {
  ...
  final destinationPuzzleHash =
      const Address('xch1m2use8pwfm3plxkvjcz880l7s9gaxacfx53k2mzcuxsnkezeu4zqldxch6')
          .toPuzzlehash();
  ...
  final otherCat = CatCoin.fromParentSpend(
    coin: otherCoin,
  ...
  test('Produces valid offert', () async {
    ...
    final driverDict = <Bytes, PuzzleInfo>{
      strToTailHash('red'): PuzzleInfo(
        {
          'type': 'CAT',
          'tail': strToTailHash('red'),
        },
      ),
      strToTailHash('blue'): PuzzleInfo(
        {
          'type': 'CAT',
          'tail': strToTailHash('blue'),
        },
      )
    };
    final chiaRequestedPayments = <Bytes?, List<Payment>>{
      strToTailHash('red').toBytes(): [
        Payment(
          100,
          acsPh,
          memos: const <String>['memo'],
        ),
        Payment(
          200,
          acsPh,
    ...
    for (final ann in chiaAnnouncements) {
      final mesasge = ann.announcementHash.toHex();
      print('hash=$mesasge');
      print(ann.toProgram().toSource());
    }
